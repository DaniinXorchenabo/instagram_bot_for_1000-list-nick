...
def extract_post_info(browser):
    """Get the information from the current(открытого) post"""
    comments = []
    clicable_herf = []
    user_commented_list = []
    if browser.find_element_by_xpath(read_xpath(extract_post_info.__name__,"comment_list")):
        ...
        print("$$$$$$$", [i.text for i in comments], comment_list)
        ...
        if len(comments) > 1:
            ...
            try:
                while False:
                    ...
                    #plas_com = comment_list.find_element_by_css_selector('ui li')
                    #plas_com = comment_list.find_elements_by_class_name("Igw0E")
                    #plas_com = comment_list.find_elements_by_xpath("//li/div")
                                      
                    pass
                    #click_element(browser, plas_com) 
                    #print(plas_com.text)  # [i.text for i in plas_com])
            except Exception:
                print("error")
            while (" comments" in comments[1].text):
                ...
                #comments = comments.find_elements_by_class_name("gElp9 rUo9f")
                #comments = [i.find_element_by_class_name("C4VMK") for i in comments]                
                #comments = [i.find_elements_by_tag_name('li') for i in comments]
                print("more_comments ", more_comments)
                if more_comments > 10:
            ...
            # in first comment
            print("more_comments", more_comments, [i.text for i in comments])
            if more_comments == 0:
                while (" comments" in comments[0].text):
                    ...
                    click_element(browser, load_more_comments_element)
                    '''
                    clicable_herf += comment_list.find_elements_by_class_name("_61Di1")
                    [click_element(browser, i) for i in clicable_herf if "View replies" in i.text]                    
                    # comment_list = post.find_element_by_tag_name('ul')
                    #comments = comment_list.find_elements_by_tag_name('li')
                    #comments = comment_list.find_element_by_css_selector('ui ui li div')
                    # user_list

                    #for i in clicable_herf:
                        #click_element(browser, i)

                    #comments += comment_list.find_elements_by_css_selector('ui ui li ui')
                    if more_comments > 10:
                        print("Won't load more than that, moving on..")
                        break
                    '''
            ...
            #comment_list = browser.find_element_by_xpath(read_xpath(extract_post_info.__name__,"comment_list"))
            clicable_herf = comment_list.find_elements_by_class_name("_61Di1")
            print("**&*&*&", [i.text for i in clicable_herf])
            [click_element(browser, i) for i in clicable_herf if "View replies" in i.text] 
            print("----", [i.text for i in clicable_herf])
            comments = comment_list.find_elements_by_class_name("C4VMK")  
            user_commented_list = [i.text.split("\n") for i in comments]
            print(*user_commented_list, sep="\n")
            user_commented_list = [i[0] for i in user_commented_list ]  # if "likeReply" not in i[-1]
            date_time = [i.find_element_by_tag_name('time').get_attribute("datetime") for i in comments]
        ...
        '''
            #comments = comment_list.find_element_by_class_name("gElp9 rUo9f")
            print("^^^ ", [i.text.split("\n")[0] for i in comments])
            #comments = [i.find_elements_by_class_name("C4VMK") for i in comments]            
            #clicable_herf = comment_list.find_elements_by_class_name("_61Di1")
            #clicable_herf = [i for i in clicable_herf if "View replies" in i.text] # .find_element_by_css_selector('div') 
            #for i in clicable_herf:
                #click_element(browser, i)            
            ...
            for comm in comments:
                try:
                    fff = time()
                    print(comm.text)
                    #local = user_commented = comm.find_element_by_tag_name(
                    #    'a')
                    if "View replies" not in comm.text and "Hide replies" not in comm.tex:
                        
                        
                        local = comm.find_element_by_css_selector('h3 a')
                        print(time()-fff, end="  ")
                        user_commented = local.get_attribute("href").split('/')
                        user_commented_list.append(user_commented[3])
                        print(time()-fff)
                except Exception:
                    print("cant get comments", time() - fff)

        print(len(user_commented_list), " comments:", *user_commented_list)
        date_time = browser.find_element_by_tag_name('time').get_attribute(
            "datetime")
        '''
    return user_commented_list, []
def check_acc_inst(browser, username):
    web_address_navigator(browser, username)
    body_elem = browser.find_element_by_tag_name('body')
    print(body_elem.text)
    if "Sorry, this page isn't available." in body_elem.text:
        return False
    else:
        return True
    #num_of_posts = get_number_of_posts(browser)
    
def my_get_nikName_comments_list(browser, photo_url, daysold=365):
    # PROFILE SCROLLING AND HARVESTING LINKS
        
    print("---------------------clicking on one photo..")
    user_commented_list = [""]
    total_user_commented_list = []
    bin_data_maska = [False]
    ...
    try:
        ...
        pastdate = datetime.now() - timedelta(days=daysold)
        date_of_pic = [datetime.strptime(i, "%Y-%m-%dT%H:%M:%S.%fZ") for i in pic_date_time]
        print("date_of_pic", *date_of_pic, sep="\n")
        #print(*[(i > pastdate) for i in date_of_pic], "---", sep="\n")
        bin_data_maska = [(i > pastdate) for i in date_of_pic]
        return list(zip(user_commented_list, bin_data_maska))
        #print(*list(zip(user_commented_list, bin_data_maska)), sep="\n")
    except NoSuchElementException:
    ...
    try:
        one_pic_elem = browser.find_element_by_xpath(
            read_xpath("extract_post_info", "one_pic_elem"))
...
def extract_information(browser, username, daysold, max_pic):
    ...
    try:
        num_of_posts = get_number_of_posts(browser)
        num_of_posts = (min(num_of_posts, max_pic)) # получение всех постов пользователя с заданного времени
...
def my_get_count_likers(browser, photo_url):
...
def users_liked(browser, photo_url, amount=100):
    photo_likers = []
    try:
        web_address_navigator(browser, 'https://www.instagram.com/daniakhorze')
        #web_address_navigator(browser, "daniakhorze")
        sleep(0.5)
        web_address_navigator(browser, photo_url)
